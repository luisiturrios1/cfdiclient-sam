...
```bash
cfdiclient-sam$ sam build --use-container
```
...
```bash
cfdiclient-sam$ sam logs -n StockCheckerFunction --stack-name cfdiclient-sam --tail
```
...
```bash
cfdiclient-sam$ pip install -r tests/requirements.txt --user
# unit test
cfdiclient-sam$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
cfdiclient-sam$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name cfdiclient-sam
```
